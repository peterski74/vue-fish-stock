<template>
  <div id="app">
    ...
    <h2>Fish Stock </h2>
    <h3>Drop this into any site/page, this is self containted code, 128kb of code gzipped</h3>
    <ul class="flex-container">
      <li v-for="item in listItems" :key="item.Id" class="flex-item">
        <h4>Fish: {{item.Title}}</h4>
        <span>Site Name: {{item.Site}}</span>
        <span>Waterway: {{item.Waterway}}</span>
...
import { baseApiURL, access_token } from '@/config/env'
    ...
    loadListItem () {
      ...
      
      {
        ...
        .then(response => {
          ...
          this.listItems = response.data.d.results
          this.totalPages = Math.floor(response.data.d.results.length / this.recordsPerPage) // Calculate total records
          this.totalRecords = response.data.d.results.length
        })
    },
    onPageChange (page) {
      this.page = page
